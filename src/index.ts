...
/** 提取对象属性类型 */
type GetObjType<T, K extends keyof T> = T extends {[key in K]: infer R} ? R : never;
...
type GetEnumArrValueType<Arr extends readonly EnumObj[]> = Arr extends readonly [
    infer First extends EnumObj,
    ...infer Rest extends EnumObj[]
]
    ? GetObjType<First, 'value'> | GetEnumArrValueType<Rest>
    : never;

// 获取枚举数组value的联合类型
export type GetEnumValueType<T> = T extends Readonly<EnumX<infer Arr>> ? GetEnumArrValueType<Arr> : never;
...
export default class EnumX<T extends readonly EnumObj[]> extends Array<EnumObj> {
    ...
    static createEnum<T extends readonly EnumObj[]>(list: T) {
